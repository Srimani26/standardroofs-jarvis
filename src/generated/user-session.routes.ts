...
/**
 * Auto-generated UserSession Routes
 *
...
import type { UserSessionHooks } from "./user-session.hooks"
...
/**
 * Set hooks for UserSession routes
 */
export function setUserSessionHooks(h: UserSessionHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["deviceType", "deviceName", "ipAddress", "lastActive", "isActive", "createdAt"] as const
...
/**
 * Create UserSession CRUD routes
 */
export function createUserSessionRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[UserSession] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.userSession.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "UserSession not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[UserSession] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.userSession.create({
        data: picked.data,
      })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.userSession.update({
        where: { id },
        data: picked.data,
      })
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[UserSession] Delete error:", error)
