...
import type { MetricHooks } from "./metric.hooks"
...
/**
 * Set hooks for Metric routes
 */
export function setMetricHooks(h: MetricHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "value", "unit", "category", "date", "createdAt"] as const
...
/**
 * Create Metric CRUD routes
 */
export function createMetricRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[Metric] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.metric.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Metric not found" } }, 404)
      }
