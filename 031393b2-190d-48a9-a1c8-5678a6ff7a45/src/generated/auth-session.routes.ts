...
/**
 * Auto-generated AuthSession Routes
 *
...
import type { AuthSessionHooks } from "./auth-session.hooks"
...
/**
 * Set hooks for AuthSession routes
 */
export function setAuthSessionHooks(h: AuthSessionHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["userId", "token", "deviceInfo", "ipAddress", "expiresAt", "createdAt"] as const
...
/**
 * Create AuthSession CRUD routes
 */
export function createAuthSessionRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[AuthSession] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.authSession.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "AuthSession not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[AuthSession] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.authSession.create({
        data: picked.data,
      })
      ...
      console.error("[AuthSession] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.authSession.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[AuthSession] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[AuthSession] Delete error:", error)
