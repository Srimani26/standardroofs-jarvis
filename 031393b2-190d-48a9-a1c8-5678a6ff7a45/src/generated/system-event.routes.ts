...
/**
 * Auto-generated SystemEvent Routes
 *
...
import type { SystemEventHooks } from "./system-event.hooks"
...
/**
 * Set hooks for SystemEvent routes
 */
export function setSystemEventHooks(h: SystemEventHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["level", "source", "message", "meta", "createdAt"] as const
...
/**
 * Create SystemEvent CRUD routes
 */
export function createSystemEventRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[SystemEvent] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.systemEvent.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "SystemEvent not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[SystemEvent] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.systemEvent.create({
        data: picked.data,
      })
      ...
      console.error("[SystemEvent] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.systemEvent.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[SystemEvent] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[SystemEvent] Delete error:", error)
