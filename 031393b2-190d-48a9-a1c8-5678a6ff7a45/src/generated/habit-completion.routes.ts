...
/**
 * Auto-generated HabitCompletion Routes
 *
...
import type { HabitCompletionHooks } from "./habit-completion.hooks"
...
let hooks: HabitCompletionHooks = {}
...
/**
 * Set hooks for HabitCompletion routes
 */
export function setHabitCompletionHooks(h: HabitCompletionHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["habitId", "date"] as const
const RELATION_FIELDS = ["habit"] as const
...
/**
 * Create HabitCompletion CRUD routes
 */
export function createHabitCompletionRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[HabitCompletion] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.habitCompletion.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "HabitCompletion not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[HabitCompletion] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.habitCompletion.create({
        data: picked.data,
      })
      ...
      console.error("[HabitCompletion] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.habitCompletion.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[HabitCompletion] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[HabitCompletion] Delete error:", error)
