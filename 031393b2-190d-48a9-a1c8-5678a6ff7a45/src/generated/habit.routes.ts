...
import type { HabitHooks } from "./habit.hooks"
...
/**
 * Set hooks for Habit routes
 */
export function setHabitHooks(h: HabitHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "icon", "color", "frequency", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["completions"] as const
...
/**
 * Create Habit CRUD routes
 */
export function createHabitRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[Habit] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.habit.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Habit not found" } }, 404)
      }
