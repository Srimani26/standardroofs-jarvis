...
import type { ReminderHooks } from "./reminder.hooks"
...
/**
 * Set hooks for Reminder routes
 */
export function setReminderHooks(h: ReminderHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["title", "message", "remindAt", "completed", "createdAt"] as const
...
/**
 * Create Reminder CRUD routes
 */
export function createReminderRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Reminder not found" } }, 404)
      }
