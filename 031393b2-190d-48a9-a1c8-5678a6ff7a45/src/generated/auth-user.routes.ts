...
/**
 * Auto-generated AuthUser Routes
 *
...
import type { AuthUserHooks } from "./auth-user.hooks"
...
/**
 * Set hooks for AuthUser routes
 */
export function setAuthUserHooks(h: AuthUserHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["username", "passwordHash", "twoFactorSecret", "twoFactorEnabled", "failedAttempts", "lockedUntil", "createdAt", "updatedAt"] as const
...
/**
 * Create AuthUser CRUD routes
 */
export function createAuthUserRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[AuthUser] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.authUser.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "AuthUser not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[AuthUser] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.authUser.create({
        data: picked.data,
      })
      ...
      console.error("[AuthUser] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.authUser.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[AuthUser] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[AuthUser] Delete error:", error)
