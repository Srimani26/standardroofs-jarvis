...
/**
 * Auto-generated DailySummary Routes
 *
...
import type { DailySummaryHooks } from "./daily-summary.hooks"
...
/**
 * Set hooks for DailySummary routes
 */
export function setDailySummaryHooks(h: DailySummaryHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["date", "summary", "stats", "createdAt"] as const
...
/**
 * Create DailySummary CRUD routes
 */
export function createDailySummaryRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[DailySummary] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.dailySummary.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "DailySummary not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[DailySummary] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.dailySummary.create({
        data: picked.data,
      })
      ...
      console.error("[DailySummary] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.dailySummary.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[DailySummary] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[DailySummary] Delete error:", error)
