...
import type { MemoryHooks } from "./memory.hooks"
...
/**
 * Set hooks for Memory routes
 */
export function setMemoryHooks(h: MemoryHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["content", "category", "importance", "tags", "metadata", "createdAt", "updatedAt"] as const
...
export function createMemoryRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.memory.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Memory not found" } }, 404)
      }
